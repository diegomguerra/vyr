...
import { Check, Sun, Moon, Sunset, Shield, ArrowRight, Timer } from "lucide-react";
...
export default function RotinaCompleta() {
  ...
  return <div className="min-h-screen bg-vyr-black">
      <LandingNav />
      ...
      <section className="pt-24 sm:pt-32 pb-10 sm:pb-16 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 mb-10 sm:mb-16">
            {sachets.map(sachet => <div key={sachet.id} className={`relative rounded-sm p-6 bg-vyr-gray-900/80 border ${sachet.borderColor} backdrop-blur-sm`}>
                ...
                <div className="space-y-2 mb-4">
                  {sachet.benefits.map((benefit, idx) => <div key={idx} className="flex items-center gap-2 text-sm">
                ...
                <div className="pt-4 border-t border-vyr-gray-700/50">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {sachet.keyIngredients.map((ing, idx) => <span key={idx} className="text-xs px-2 py-1 rounded-sm bg-vyr-gray-800/50 text-vyr-gray-300 font-mono">
                        {ing}
                      </span>)}
                  </div>
                </div>
                ...
                {/* Link to details */}
                <Link to={`/produtos/${sachet.id}`} className="mt-4 inline-flex items-center gap-1 text-sm text-vyr-gray-400 hover:text-vyr-white transition-colors">
      ...
      <section className="py-12 sm:py-20 bg-gradient-to-b from-vyr-black to-vyr-gray-900">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 mb-8 sm:mb-12">
            {plans.map(plan => <button key={plan.id} onClick={() => setSelectedPlan(plan.id)} className={`relative rounded-sm p-1 transition-all duration-300 ${selectedPlan === plan.id ? "bg-vyr-white scale-[1.02]" : "bg-vyr-gray-700/50 hover:bg-vyr-gray-700"}`}>
                {plan.highlight && <div className="absolute -top-3 left-1/2 -translate-x-1/2 z-10">
                ...
                <div className={`relative bg-vyr-gray-900 rounded-sm p-6 h-full ${selectedPlan === plan.id ? "border border-vyr-gray-500/50" : ""}`}>
                  ...
                  <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${selectedPlan === plan.id ? "border-vyr-white bg-vyr-white" : "border-vyr-gray-600"}`}>
                    {selectedPlan === plan.id && <Check className="w-4 h-4 text-vyr-black" />}
                  </div>
                </div>
              </button>)}
          </div>
          ...
          <div className="bg-vyr-gray-900/50 rounded-sm p-5 sm:p-8 border border-vyr-gray-700/50 mb-6 sm:mb-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              {["VYR BOOT + VYR HOLD + VYR CLEAR", "Acesso à plataforma digital", "Registro de doses e evolução", "Histórico de métricas pessoais", "Entrega expressa grátis", "Garantia de 30 dias"].map((item, idx) => <div key={idx} className="flex items-center gap-2 sm:gap-3">
